...
const cards = [
  ...
  { key: "rejected", label: "Rejected courts" }
] as const;

type Stats = Record<(typeof cards)[number]["key"], number>;

export function AdminDashboardStats() {
  const [stats, setStats] = useState<Stats>({ approved: 0, pending: 0, rejected: 0, total: 0 });
  ...
  useEffect(() => {
    async function loadStats() {
      ...
      const { data, error: statsError } = await supabase.from("courts").select("status");
      ...
      setStats({
        ...
        rejected: rows.filter((court) => court.status === "rejected").length,
        total: rows.length
      });
      setIsLoading(false);
    }
